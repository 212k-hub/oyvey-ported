...
public final class ElytraSwap extends Module {
   ...
   private final BooleanSetting moveToSlot = new BooleanSetting(EncryptedString.of("Move to slot"), true)
      .setDescription("If elytra is not in hotbar it will move it from inventory to preferred slot");
   private final NumberSetting elytraSlot = new NumberSetting(EncryptedString.of("Elytra Slot"), 1.0, 9.0, 9.0, 1.0)
      .getValue(EncryptedString.of("Your preferred elytra slot"));
   ...
   public ElytraSwap() {
      super(
         ...
         EncryptedString.of("Seamlessly swap between an Elytra and a Chestplate with a configurable keybinding"),
         -1,
         Category.COMBAT
      );
         ...
         if (this.mc.player != null) {
            ...
            if (this.isSwapping) {
               ...
               if (this.mc.player.getInventory().getArmorStack(EquipmentSlot.CHEST.getEntitySlotId()).isOf(Items.ELYTRA)) {
                  itemPredicate = item -> item instanceof ArmorItem && ((ArmorItem)item).getSlotType() == EquipmentSlot.CHEST;
               } else {
                  itemPredicate = item -> item.equals(Items.ELYTRA);
               }

               if (!this.isItemSwapped) {
                  if (!InventoryUtil.swapItem(itemPredicate)) {
                     ...
                     this.mc
                        .interactionManager
                        .clickSlot(this.mc.player.currentScreenHandler.syncId, 9, this.elytraSlot.getIntValue() - 1, SlotActionType.SWAP, this.mc.player);
                     this.swapCounter = 0;
                     return;
                  }
